...
describe('successfully parses', () => {
  const vectors: [string, string, number, number, number, number[]][] = [
    ...
    [
      'tabs and CR in the header',
      'P6\t1\t1\t255\r\x02\x03\x05',
      1,
      1,
      255,
      [2, 3, 5],
    ...
    [
      'comment inside whitespace after magic',
      'P6 # 42\n\n1\r 1\n36\n\0\0\0',
      1,
      1,
      36,
    ...
    [
      'comment inside whitespace after width',
      'P6 \n1\t#23\r 1\n36\n\0\0\0',
      1,
      1,
      36,
    ...
    [
      'comment in whitespace after height',
      'P6 1 1 #foo\n 36\n\0\0\0',
      1,
      1,
      36,
  ...
  for (const [description, data, width, height, maxval, expected] of vectors) {
    test(description, () => {
      ...
      expect(pixmap).toEqual(
        expect.objectContaining({ width, height, maxval }),
      );
...
describe('throws an error when parsing', () => {
  const vectors = [
    ...
    [
      'comment but no whitespace between magic and width',
      'P6# \n1 1 1\n\0\0\0',
    ],
    [
      'comment but no whitespace between width and height',
      'P6 1# \n1 1\n\0\0\0',
    ],
  ...
  for (const [description, vector] of vectors) {
    test(description, () =>
      expect(() => PortablePixmap.parse(Buffer.from(vector, 'binary'))).toThrow(
        PPMParseError,
      ),
    );
  }
...
describe('toRGB88888', () => {
  describe('converts the image correctly', () => {
    const vectors: [string, number, number, number, number[], number[]][] = [
      ...
      [
        ...
        2,
        255,
        [0, 123, 200, 255, 254, 253, 1, 2, 3, 55, 66, 77],
    ...
    for (const [description, width, height, maxval, data, rgb] of vectors) {
      test(`when ${description}`, () => {
        const pixmap = new PortablePixmap(
          width,
          height,
          maxval,
          Buffer.from(data),
  ...
  it('writes in-place to a given array', () => {
    ...
    expect(destination).toEqual(
      new Uint8ClampedArray([191, 128, 64, 255, 0, 255, 128, 255]),
    );
  });
  ...
  it('refuses to write into a too-small array', () => {
    ...
    expect(() => pixmap.toRGBA8888(destination)).toThrow(
